...
var SmartMultiFiled = (function() {
    ...
    function formHtml() {
        html = '<tr id="row_' + rowcount + '">';    
            ...
            html += '<td><textarea required name="EMPLOYEE_PREV_JOB[]" id="EMPLOYEE_PREV_JOB_' + rowcount + '" class="form-control autocomplete_txt" autocomplete="off" cols="70" rows="2"></textarea></td>';
            html += '<td><input required type="number" name="EMPLOYEE_PREV_JOB_YEARS[]" id="EMPLOYEE_PREV_JOB_YEARS_' + rowcount + '" class="form-control autocomplete_txt" autocomplete="off"></td>';
            html += '<td><input required type="date" name="EMPLOYEE_PREV_ENDO[]" id="EMPLOYEE_PREV_ENDO_' + rowcount + '" class="form-control autocomplete_txt" autocomplete="off"></td>';    
...
//! Select Current Data
  ...
  $.ajax({               //                                  |                              
    ...
    $("#add_com").on('submit', function(e) {
        ...
        let EMPLOYEE_PREV_JOB = $('.EMPLOYEE_PREV_JOB').val(); 
        let EMPLOYEE_PREV_JOB_YEARS = $('.EMPLOYEE_PREV_JOB_YEARS').val();  
        let EMPLOYEE_PREV_ENDO = $('.EMPLOYEE_PREV_ENDO').val();  

        if(EMPLOYEE_PREV_JOB == ''||EMPLOYEE_PREV_JOB_YEARS == ''|| EMPLOYEE_PREV_ENDO == '') {
            Swal.fire( 
            'Error',
            'No Input Found!',
            'error');
        ...
        $.ajax({
                url:"../controller/controller_insert_com.php",  
                method:"POST",  
    ...
    //! Create Edit and Delete Function
        ...
        $('body').on('click', '#btn-edit', function () {
            ...
            $.ajax({
                url:"../controller/controller_crud_tblemplist_com.php",
                type: "POST",
                data: {
                    id: id,
                    mode: 'edit' 
                ...
                success: function(result){
                $('#id').val(result.id);
                $('#EMPLOYEE_PREV_JOB').val(result.EMPLOYEE_PREV_JOB);
                $('#EMPLOYEE_PREV_JOB_YEARS').val(result.EMPLOYEE_PREV_JOB_YEARS);
                $('#EMPLOYEE_PREV_ENDO').val(result.EMPLOYEE_PREV_ENDO);
        ...
        $('body').on('click', '#btn-delete', function () {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    //
                    $.ajax({
                        url:"../controller/controller_crud_tblemplist_com.php", //<----- Create this controller file
                        type: "POST",
                        data: {
                            id: id,
                            mode: 'delete' 
        ...
        if ($('.selectedRow:checked').length > 0) { // at-least one checkbox checked
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "../controller/controller_bulkDelete_tblemplist_com.php",
                        data: {
                            data_ids: ids_string
                        },
