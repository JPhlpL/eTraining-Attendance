...
var SmartMultiFiled = (function() {
    ...
    function formHtml() {
        html = '<tr id="row_' + rowcount + '">';    
            ...
            html += '<td><textarea required name="EMPLOYEE_TECH_SKILL[]" id="EMPLOYEE_TECH_SKILL_' + rowcount + '" class="form-control autocomplete_txt EMPLOYEE_TECH_SKILL" autocomplete="off" cols="70" rows="2"></textarea></td>';
...
//! Select Current Data
  ...
  $.ajax({               //                                  |                              
    ...
    $("#add_tech").on('submit', function(e) {
        e.preventDefault();

        let EMPLOYEE_TECH_SKILL = $('.EMPLOYEE_TECH_SKILL').val();

        if(EMPLOYEE_TECH_SKILL == '') {
            Swal.fire( 
            'Error',
            'No Input Found!',
            'error');
        ...
        $.ajax({
                url:"../controller/controller_insert_tech_skill.php",  
                method:"POST",  
                ...
                { 
                    ...
                    {  
                        ...
                        Table.fnDraw(false);
                        $('.EMPLOYEE_TECH_SKILL').trigger("reset");
                    });
                },
    ...
    //! Create Edit and Delete Function
        ...
        $('body').on('click', '#btn-edit', function () {
            ...
            $.ajax({
                url:"../controller/controller_crud_tblemplist_tech_skill.php",
                type: "POST",
                data: {
                    id: id,
                    mode: 'edit' 
                ...
                success: function(result){
                $('#id').val(result.id);
                $('#EMPLOYEE_TECH_SKILL').val(result.EMPLOYEE_TECH_SKILL);
        ...
        $('body').on('click', '#btn-delete', function () {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    //
                    $.ajax({
                        url:"../controller/controller_crud_tblemplist_tech_skill.php", //<----- Create this controller file
                        type: "POST",
                        data: {
                            id: id,
                            mode: 'delete' 
        ...
        if ($('.selectedRow:checked').length > 0) { // at-least one checkbox checked
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "../controller/controller_bulkDelete_tblemplist_tech_skill.php",
                        data: {
                            data_ids: ids_string
                        },
    ...
    var dataTable = $('#autocomplete_table').DataTable({
        "ajax": "../controller/controller_fetch_tblemplist_tech_skill.php",
        "type": "POST",
        "processing": true,
        "dom": 'Bfrtp',
        "serverSide": true,
